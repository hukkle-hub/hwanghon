...
AIN=os.environ.get('AIN_GLB','/home/user/hwanghon/art/3d/props/scarecrow.glb'); SCYTHE=None; KAY=SD+'/Rogue.glb'
GRIP=0.95        # 낫 자루에서 오른손 그립 위치 (자루 끝 기준 m)
LEFT_OFF=0.36    # 왼손이 잡는 지점: 오른손에서 날 쪽으로 (m)
CLIPS=[('idle','Unarmed_Idle'),('walk','Walking_A'),('atk_hammer','2H_Melee_Attack_Chop'),('atk_bolt','Unarmed_Melee_Attack_Punch_A'),('atk_scythe','2H_Melee_Attack_Spin'),('hit','Hit_A'),('stagger','Hit_B'),('down','Lie_Down'),('lie','Lie_Idle'),('up','Lie_StandUp'),('death','Death_B'),('kick','Unarmed_Melee_Attack_Kick')]
TWO_HAND=set()
...
J={ 'Hips':B(0,1.55), 'Spine':B(0,1.75), 'Spine1':B(0,1.95), 'Spine2':B(0,2.15), 'Neck':B(0,2.42), 'Head':B(0,2.52), 'HeadTop_End':B(0,2.8,0.0) }
for s_,sg in (('Left',1),('Right',-1)):
    J[s_+'Shoulder']=B(sg*0.15,2.40); J[s_+'Arm']=B(sg*0.45,2.38); J[s_+'ForeArm']=B(sg*0.62,1.75,-0.06); J[s_+'Hand']=B(sg*0.78,1.25,0.0); J[s_+'HandEnd']=B(sg*0.82,1.0,0.01)
    J[s_+'UpLeg']=B(sg*0.22,1.50); J[s_+'Leg']=B(sg*0.26,0.80,0.02); J[s_+'Foot']=B(sg*0.33,0.20,-0.02); J[s_+'ToeBase']=B(sg*0.35,0.06,0.15); J[s_+'Toe_End']=B(sg*0.35,0.03,0.30)
J['RightHandSlot']=B(-0.80,1.15,0.05); J['RightHandSlotEnd']=B(-0.80,1.15,0.16)
...
R={'Hips':0.32,'Spine':0.30,'Spine1':0.30,'Spine2':0.34,'Neck':0.14,'Head':0.22}
for s in ('Left','Right'): R.update({s+'Shoulder':0.22,s+'Arm':0.16,s+'ForeArm':0.14,s+'Hand':0.14,s+'UpLeg':0.20,s+'Leg':0.17,s+'Foot':0.16,s+'ToeBase':0.14})
...
for i,n in enumerate(names):
    if 'Arm' in n or 'Hand' in n: D[:,i]+=np.where(X<0.38,0.4,0.0)
    if 'Leg' in n or 'Foot' in n or 'Toe' in n: D[:,i]+=np.where(Y>1.6,0.5,0.0)
...
hip_scale=(1.50-0.20)/(0.519-0.145)
# ---------- 4. 리타게팅 ----------
...
def retarget(action, clip):
    ...
    for f in range(f0,f1+1):
        ...
        if False: pass
        elif False: errs.append(carry(world, path_spec(PATHS[clip], (f-f0)/max(1,(f1-f0)))))
