...
def mat(name, col, rough=0.6, metal=0.0, emis=None, estr=0):
    m=bpy.data.materials.new(name); m.use_nodes=True; b=m.node_tree.nodes['Principled BSDF']; b.inputs['Base Color'].default_value=(*col,1); b.inputs['Roughness'].default_value=rough; b.inputs['Metallic'].default_value=metal
    if emis: b.inputs['Emission Color'].default_value=(*emis,1); b.inputs['Emission Strength'].default_value=estr
    return m
def reset():
    bpy.ops.wm.read_factory_settings(use_empty=True)
    return { 'steel':mat('steel',(0.55,0.57,0.62),0.35,0.9), 'rust':mat('rust',(0.36,0.20,0.12),0.85,0.5), 'dark':mat('dark',(0.10,0.10,0.12),0.6,0.7), 'wood':mat('wood',(0.30,0.20,0.12),0.9), 'leather':mat('leather',(0.24,0.15,0.09),0.9), 'brass':mat('brass',(0.62,0.48,0.22),0.4,0.9), 'bone':mat('bone',(0.62,0.57,0.46),0.7), 'ash':mat('ash',(0.30,0.30,0.32),0.5,0.6), 'red':mat('red',(0.45,0.08,0.06),0.6,0.2,(1,0.2,0.1),0.8), 'teal':mat('teal',(0.35,0.55,0.55),0.3,0.8) }
parts=[]
...
# 1. 부식된 사형 집행자 — 두꺼운 녹슨 대낫, 사슬 감은 자루, 이 빠진 넓은 날
M=reset()
...
blade(1.78,1.84,0.06,0.02,M['rust']); b=blade(0,0.95,0.16,0.012,M['rust'],curve=-0.18,back=True); b.rotation_euler=(0,-math.pi/2+0.35,0); b.location=(0.02,0,1.80)
for i in range(4): box((0.35+i*0.15,0.0,1.79+0.02*i),(0.05,0.03,0.05),M['dark'])   # 이 빠진 자국
...
blade(2.08,2.45,0.045,0.014,M['steel']); box((0.05,0,1.95),(0.10,0.01,0.22),M['red'],rot=(0,0,0.5)); box((-0.06,0,1.90),(0.08,0.01,0.28),M['red'],rot=(0,0,-0.4))
export('w_ruin_spear')
# 4. 녹슨 집행검 — 넓은 한손 검(그립 0.75 중심)
M=reset()
cyl(0.55,0.62,0.028,0.02,M['brass'],8); cyl(0.62,0.88,0.019,0.019,M['leather'],10); box((0,0,0.90),(0.20,0.035,0.04),M['brass']); blade(0.92,1.72,0.055,0.014,M['rust']); box((0.03,0,1.20),(0.03,0.02,0.05),M['dark']); box((-0.04,0,1.45),(0.03,0.02,0.04),M['dark'])
export('w_rust_sword')
