...
mod=body.modifiers.new('Ain_existing_skeleton','ARMATURE');mod.object=arm
...
with open(os.path.join(out,'rig_report.json'),'w') as f:json.dump(dict(status='EXPERIMENT_NOT_RUNTIME_APPROVED',unweighted_vertices=orphans,clips=[a.name for a in bpy.data.actions],evidence=evidence,limitations=['Distance-based initial weights; no deformation retopology','No finger bones in existing skeleton','No separate garments yet','Pose landmarks have not been fitted to candidate anatomy']),f,indent=2)
