...
async function main(){const store=new Store(null),app=createPartyServer({store}),address=await app.listen(0,'127.0.0.1'),errors=[],sockets=[];
 const dom=new JSDOM(fs.readFileSync(path.join(root,'party.html'),'utf8'),{url:`http://127.0.0.1:${address.port}/party.html`,runScripts:'outside-only',pretendToBeVisual:true}),w=dom.window,$=id=>w.document.getElementById(id);
 ...
 try{
  ...
  $('account-id').value='dom_player';$('password').value='dom-password-2026';$('account-register').click();await wait(()=>$('character-create').hidden===false&&$('login').hidden);assert.ok(w.document.querySelector('code').textContent.length>20);await click('보관했습니다');$('character-name').value='화면검증아인';$('character-submit').click();await wait(()=>$('hub-tools').hidden===false);const id=store.playerNamed('화면검증아인');
  ...
  console.log(JSON.stringify({passed:true,checks:['registration and one-time recovery code','character name','quest accept and claim','dungeon unlock','inventory filter and vault deposit','crafting ingredient debit','trait applied on server','volume setting persistence','social and account menus'],limitations:['DOM only; no CSS layout, WebGL or mobile visual verification']},null,2));
