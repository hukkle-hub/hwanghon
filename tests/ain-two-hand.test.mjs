...
test('repaired Ain: two palm contacts, bounded wrists, fixed limb lengths and continuous swings',async t=>{
 ...
 for(const variant of ['idle','run','guard','attack1','attack2','attack3','smash','ult','skill1','skill2','skill3','skill4','counter','counterPerfect','exec','skill1Target','skill3Target','ultTarget','skill1High','attack1Target','attack2Target','attack3Target','smashTarget','counterTarget','execTarget']){
  ...
  for(let i=0;i<=240;i++){
   ...
   const target=variant.endsWith('High')?new T.Vector3(.09,3.15,.89):variant.endsWith('Target')?new T.Vector3(.11+.05*Math.sin(i/30),2.58+.04*Math.sin(i/24),.58+.08*Math.cos(i/30)):null;
