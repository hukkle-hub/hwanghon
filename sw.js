...
function isCodePath(a){ return /\.(html|css|js|json|webmanifest)$/.test(a); }
self.addEventListener('install', function(e){
  e.waitUntil(caches.open(VERSION).then(function(c){
    var list=ASSETS.filter(function(a,i){ return ASSETS.indexOf(a)===i && isCodePath(a); }), failed=[];
    return Promise.all(list.map(function(a){
      /* no-cache: 서버에 되묻기만(안 바뀌었으면 304) — reload 는 매번 통째로 받았다 */
      return c.add(new Request(a, {cache:'no-cache'})).catch(function(){ failed.push(a); });
    })).then(function(){
...
self.addEventListener('activate', function(e){
  e.waitUntil(caches.keys().then(function(keys){ return Promise.all(keys.filter(function(k){ return k.indexOf('tw-')===0 && k!==VERSION; }).map(function(k){ return caches.delete(k); })); })
    ...
    .then(function(cs){ cs.forEach(function(c){
      /* 새로고침은 화면(ui.js)이 정한다 — 출격 중이면 끝난 뒤로 미룬다. 전에는 여기서 navigate 로 던전 한복판에서도 다시 불러왔다 */
      try{ c.postMessage({type:'TW_UPDATED', version:VERSION}); }catch(err){}
    }); }));
});
...
self.addEventListener('fetch', function(e){
  ...
  if (url.origin === location.origin){
    if (isCode(url, req)){
      ...
      /* 아트: 서버에 되묻고(no-cache → 안 바뀌면 304, 본문은 브라우저 캐시), 실패(오프라인)면 캐시.
         캐시에는 ETag 가 달라졌을 때만 새로 쓴다(4 MB GLB 를 매번 다시 쓰지 않게). 부분 응답(206)은 캐시하지 않는다 */
      e.respondWith(fetch(req, {cache:'no-cache'}).then(function(res){
        if (res && res.status === 200){ var cp=res.clone();   /* 본문을 화면이 먼저 읽기 전에 복제 */
          caches.open(VERSION).then(function(c){ return c.match(req).then(function(old){
            var et=cp.headers.get('ETag'); if (old && et && old.headers.get('ETag') === et) return; return c.put(req, cp); }); }).catch(function(){}); }
        return res;
