...
const WEAPON_OF={kain:'w_kain_greatsword',ryu:'w_ryu_dagger',sera:'w_sera_flask'};
let _blobTex;
function blobTex(){
...
export class Animated{
 constructor(asset,scene,isPlayer=false,ownsResources=false,weaponAsset=null,character=null){this.owned=new Set();if(ownsResources)asset.scene.traverse(o=>{if(o.geometry)this.owned.add(o.geometry);for(const m of (Array.isArray(o.material)?o.material:[o.material]))if(m)this.owned.add(m);});this.model=clone(asset.scene);this.model.scale.setScalar(isPlayer?1.14:1);   /* 캐릭터 크기 — js/game3d.js CHAR_SCALE 과 같게 */
  ...
  else if(isPlayer&&['kain','ryu','sera'].includes(character))asset={...asset,animations:smoothCharacterClips(character,asset.animations,{...globalThis.TW_DUNGEONS?.RULES?.motion?.clipContacts,...globalThis.TW_DUNGEONS?.RULES?.motion?.clipContactsByChar?.[character]})};
  this.root=new T.Group();this.root.add(this.model);scene.add(this.root);this.mixer=new T.AnimationMixer(this.model);this.clips=Object.fromEntries(asset.animations.map(c=>[c.name,c]));this.actions={};this.current=null;this.key=null;this.model.traverse(o=>{if(o.isMesh){o.castShadow=true;o.frustumCulled=false;}});if(isPlayer){/* 쥔 손 모프(docs/design/94) — 그립 노드보다 먼저 */this.handGrip=character&&character!=='ain'?gripHands(this.model,character):null;let slot;this.model.traverse(o=>{if(o.isBone&&/RightHandSlot/.test(o.name))slot=o;});/* 다시 리깅한 캐릭터: 옛 관절 자리에 무기를 단다 (docs/design/77) *//* 솔로와 같은 손 그립(손바닥 중심, docs/design/93) — 없으면 옛 관절 자리 */const L=globalThis.TW_LOOKS,grab=b=>{if(!b)return b;if(L&&L.anchor)return L.anchor(T,b);const an=b.userData.rerigAnchor;if(an){const n=new T.Object3D();n.name=b.name+'Anchor';n.position.set(an[0],an[1],an[2]);b.add(n);return n;}return b;};slot=grab(slot);if(slot){this.model.updateMatrixWorld(true);const spec=L&&L.WEAPON&&L.WEAPON[WEAPON_OF[character]],grip=spec&&spec.grip!=null?spec.grip:.75;const mount=(s,src)=>{const group=new T.Group();if(character==='ain')group.add(mountAinScythe(src));else{src.position.y=-grip;group.add(src);}group.scale.setScalar(1/s.getWorldScale(new T.Vector3()).x);s.add(group);if(spec&&spec.hand2)s.userData.hand2=spec.hand2;return group;};this.weapon=mount(slot,clone(weaponAsset.scene));/* 류 쌍단검: 왼손에도 */if(character==='ryu'){let ls;this.model.traverse(o=>{if(o.isBone&&/LeftHandSlot/.test(o.name))ls=o;});ls=grab(ls);if(ls)this.offhand=mount(ls,clone(weaponAsset.scene));}this.trail=new WeaponTrail(scene);}this.rig=(character==='ain'?makeAinRigAdapter:makeRigAdapter)(this.model,this.root,slot,{twoHand:character==='kain',handGrip:this.handGrip});/* 팔 뼈 제자리 돌기 묶기(docs/design/99) */this.armBlend=character!=='ain'?createArmBlend(this.model):null;}
 ...
 update(data,dt,age){this.root.position.lerp(pos(data.x,data.y),1-Math.exp(-dt*22));
  if(this.blob)this.blob.position.set(this.root.position.x,.035,this.root.position.z);const yaw=Math.PI/2-(data.aim||0),delta=Math.atan2(Math.sin(yaw-this.root.rotation.y),Math.cos(yaw-this.root.rotation.y));this.root.rotation.y+=delta*Math.min(1,dt*18);this.armBlend?.restore();this.rig?.restore();
  ...
  this.mixer.update(dt);this.rig?.apply(data.action?{...data.action,elapsed:Math.min(data.action.duration,data.action.elapsed+age)}:gesture,data.moving,data.guard,dt,this.current?.getClip().name,this.aimTarget||null);this.armBlend?.apply(dt);
