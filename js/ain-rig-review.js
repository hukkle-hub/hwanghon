...
// Conservative, damped grip correction. Never sacrifice the authored shoulder
// and elbow pose to reach a geometrically possible but anatomically bad target.
export function solveArm(upper,lower,end,target) {
  if(!upper||!lower||!end)return Infinity;
  const base=[upper.quaternion.clone(),lower.quaternion.clone()];
  for(let i=0;i<12;i++){
    rotateToward(lower,end,target,.2);rotateToward(upper,end,target,.2);
    for(const [j,bone] of [upper,lower].entries()){
      const angle=base[j].angleTo(bone.quaternion),limit=THREE.MathUtils.degToRad(j?55:25);
      if(angle>limit)bone.quaternion.copy(base[j].clone().slerp(bone.quaternion,limit/angle));
...
export function makeRigAdapter(model,root,slot) {
  ...
  const restCorrections=new Map(), anchors={}, previous=new Map(), diagnostics={gripError:0,footError:0,maxArmCorrection:0};
    ...
    if(slot&&(a||guard)){
      ...
      if(left&&right&&upper&&lower){
        function gripTarget(){
          ...
          // Preserve the authored grip along the shaft. Projecting the SHOULDER
          // onto it switched grip sides mid-swing and folded the elbow backwards.
          const hand=left.getWorldPosition(new THREE.Vector3());
          const off=THREE.MathUtils.clamp(hand.sub(pos).dot(axis),-0.55,0.5);
        ...
        if(excess>0){
          ...
          solveArm(bones.RightArm,bones.RightForeArm,right,right.getWorldPosition(new THREE.Vector3()).add(pull));
        ...
        solveArm(upper,lower,left,target);
        diagnostics.gripError=left.getWorldPosition(new THREE.Vector3()).distanceTo(target);
      }
    }else diagnostics.gripError=0;
    // Filter correction DELTAS, not animated poses: attack timing stays with the
    // clip, while IK cannot snap between opposite elbow solutions. Release the
    // correction gradually on attack -> idle. dt=0 means deterministic scrubbing.
    diagnostics.maxArmCorrection=0;
    for(const n of ['LeftArm','LeftForeArm','LeftHand','RightArm','RightForeArm','RightHand']){
      const b=bones[n];if(!b)continue;
      const base=(restCorrections.get(b)||b.quaternion).clone().normalize();
      let delta=base.clone().invert().multiply(b.quaternion).normalize();
      if(dt>0){
        const prev=previous.get(n)||new THREE.Quaternion();
        const angle=prev.angleTo(delta),alpha=1-Math.exp(-Math.min(dt,.05)/.045);
        const step=THREE.MathUtils.degToRad(300)*Math.min(dt,.05);
        delta=prev.clone().slerp(delta,Math.min(alpha,angle>1e-8?step/angle:1));
        previous.set(n,delta.clone());
      }else previous.delete(n);
      keep([n]);b.quaternion.copy(base.multiply(delta));
      diagnostics.maxArmCorrection=Math.max(diagnostics.maxArmCorrection,new THREE.Quaternion().angleTo(delta));
    }
    model.updateWorldMatrix(true,true);
    if(slot&&(a||guard)&&bones.LeftHand){
      const pos=slot.getWorldPosition(new THREE.Vector3()),axis=new THREE.Vector3(0,1,0).applyQuaternion(slot.getWorldQuaternion(new THREE.Quaternion()));
      const hand=bones.LeftHand.getWorldPosition(new THREE.Vector3());
      pos.addScaledVector(axis,THREE.MathUtils.clamp(hand.clone().sub(pos).dot(axis),-.55,.5));
      diagnostics.gripError=hand.distanceTo(pos);
    }
