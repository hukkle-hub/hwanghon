  ...
  var RULES = {
    ...
    motion: { clipContacts:{attack1:0.38,attack2:0.52,attack3:0.50,smash:0.30,ult:0.22,skill1:0.50,skill2:0.50,skill3:0.55,skill4:0.50,counter:0.48,exec:0.18},
      ...
      /* 류 궁극기: 공통 .22 면 준비 자세에서 판정이 뜬다 — 돌진·베기는 .27~.59, 최고속 .46.
         카인 스킬1(해머 스윙)은 내려치는 순간, 류 스킬3(쌍날 회전)은 왼날 .62·오른날 .72 사이 (docs/design/76) */
      clipContactsByChar:{ain:{attack3:0.48,skill1:0.24,skill2:0.23,counter:0.44},kain:{skill1:0.73},ryu:{ult:0.46,skill3:0.65}}
   /* 판정은 클립에서 «날 끝이 제일 빠른» 시각이어야 한다. 안 그러면 휘두르는
      ...
      /* 아인은 스킬만 전용 박자를 갖고 평타는 공용 light 를 쓴다. 그래서 클립이
         행동 시간에 밀려 들어가 «촐싹» 댄다 — 디렉터: 「공격 모션이 촐싹대고」.
         재 보면 (speed=1.0 기준):
           평1 1.70배 · 평2 2.53배 · 평3 1.33배 · 스매시 2.52배 · 카운터 1.93배
         스킬은 0.79~0.82배로 오히려 여유가 있다 — 전용 박자가 있었으니까.

         **평타에도 전용 박자를 주려다 되돌렸다.** 행동 시간을 15% 만 늘려도
         d03 수문기 2페이즈에서 기준 봇이 죽는다 (보스 HP 97,677 남기고 hp=0,
         tests/expedition-flow). 전투 시간을 늘리면 그만큼 DPS 가 빠지는데 그
         던전이 그 여유를 안 갖고 있다. 모션 문제를 난이도 변경으로 갚는 셈이라
         그만뒀다 — 균형까지 같이 손보는 건 디렉터 판단이다.
         대신 아래 clipSpan 만 쓴다. 전투 시간을 1 초도 안 건드린다. */
      /* 아인(큰 낫) 전용 박자. 다른 캐릭터는 위의 공통 값을 쓴다.
         무거운 기술은 «시간을 주고 크게» 휘두른다 — 68번 문서에서 밝혀졌듯
         같은 시간 안에서는 관절 각속도 상한(표본당 8° ≈ 2900°/s)에 걸려
         더 크게 못 휘두른다. 스매시 0.96→1.15초, 궁극기 1.62→1.95초.
         hit 은 duration 대비 비율을 유지한다(0.417 / 0.494). */
      characterProfiles:{ain:{
        ...
        ult:{hit:.96,active:.18,duration:1.95,cancel:1.66}
      }},
      /* 클립의 «뒤끝» 을 얼마나 쓸지. 1 이면 클립 전체를 행동 시간에 편다.
         배속 = 쓰는 클립 길이 ÷ 행동 시간이므로, 꼬리를 자르면 «행동 시간을
         건드리지 않고» 배속이 내려간다. 그래서 균형에 영향이 없다 (위 주석 참고).
         여운은 어차피 대기 자세로 섞여 나가며 가려지는 구간이고, 접점
         (clipContacts: 평2 .44 · 스매시 .78 · 카운터 .48) 뒤로는 남겨 둔다.
           평1 1.70→1.50 · 평2 2.53→1.57 · 카운터 1.93→1.64
         평3 은 1.33 이라 그대로 둔다.
         **스매시는 이 방법으로 못 고친다.** 판정이 클립의 .78 지점이라 자를
