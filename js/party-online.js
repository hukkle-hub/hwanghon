...
class RaidView{
 ...
 makeBoss(raid){if(this.boss)this.boss.dispose(this.scene);const asset=this.A.procedural==='relay'?createRelayBoss():this.A.procedural==='pump'?createPumpBoss():this.A.id==='marsh'?assets.marsh:assets.dummy;this.boss=new Animated(asset,this.scene,false,this.A.procedural==='pump');this.boss.model.scale.setScalar((this.A.bossScale||1.22)*(this.A.scale||1));   /* 보스 크기 — js/game3d.js BOSS_SCALE 과 같게 */this.boss.root.position.copy(pos(raid.boss.x,raid.boss.y));this.boss.mats=[];this.boss.model.traverse(o=>{if(o.isMesh){o.material=o.material.clone();this.boss.owned.add(o.material);if(o.material.emissive){o.userData.em=o.material.emissive.clone();this.boss.mats.push(o);}}});this.phase=raid.phase;}
