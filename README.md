...
| `js/looks.js` · `art/3d/weapons/` | **장비 외형 반영.** 주무기 5종 모델 교체(`tools/3d/build_weapons.py` 절차 생성), 단도는 허리·갈고리 낫은 등, 방어구·장신구는 뼈에 붙는 조각. 던전 입장 시 저장 장비로 구성, `viewer.html?equip=1&fit=1` 장착 모드, 인벤토리 「3D 외형」 미리보기(교체 즉시 갱신). 검수: `docs/design/11-equipment-looks-review.md` |
