...
const CHAR='ain';                                   // 온라인은 아직 아인 전용 (장비 표도 낫 한정)
const TABLE=C.skills[CHAR], ULT=C.skills[CHAR+'Ult'];
const DEFS=[...TABLE, ULT];
const byId=Object.fromEntries(DEFS.map(k=>[k.id,k]));
...
const spent=p=>DEFS.reduce((n,k)=>{const s=entry(p,k.id);return n+(s.lv-1)+(s.br?1:0);},0);
...
/* 레이드에 넘길 실제 수치 */
function resolve(p){
  const levels={};for(const k of DEFS)levels[k.id]=entry(p,k.id);
  return SM.applyAll(TABLE,ULT,levels);
}
/* 화면용: 정의 + 현재/다음 레벨 수치 */
function view(p){
  const pts=points(p);
  return {
    character:CHAR, max:SM.MAX, branchLevel:SM.BR_LV, resetCost:RESET_COST, points:pts,
    skills:DEFS.map(k=>{
...
function up(p,id){
  const k=byId[id];if(!k)throw Error('기술을 확인하세요.');
...
module.exports={resolve,view,up,branch,reset,points,entry,RESET_COST,CHAR};
