...
/* 스매시 «타점» — 솔로(js/combat.js)와 같은 규칙. 날이 스윙의 어느 지점에서, 날의 어디로 맞았나 */
const SWING_POINT=require('../js/swing-point.js');
/* 스킬 모션 이벤트(다단 접점·투척→폭발) — 솔로(js/combat.js)와 같은 사건표. docs/design/89 */
const SKILL_EVENTS=require('../js/skill-events.js');
...
class Raid{
 ...
 input(id,msg){const p=this.players.get(id);if(!p||!p.connected||!this.alive(p)||!['explore','fight'].includes(this.state))return;
  ...
  /* 카운터 탭 (docs/design/78): 창 안이면 공격으로 받아 카운터, 밖이면 누르는 동안 방어. 방향 회피는 손님이 'dodge' 로 보낸다 */
  if(msg.type==='counter'){if(this.canCounter(p))msg={type:'attack'};else{this.input(id,{type:'guard',on:true});return;}}
  if(msg.type==='opening'){this.useOpening(p);return;}
  ...
  if(msg.type==='skill'){if(!Number.isInteger(msg.index)||msg.index<0||msg.index>3)return;const k=p.skills[msg.index];if(!k||p.cds[msg.index]>0||p.st<k.st)return;
   ...
   else if(k.buff){p.buffT=k.buff.dur;p.buffReduce=k.buff.reduce;if(k.ev?.type==='heal'){const h=Math.max(0,Math.min(p.maxHp-p.hp,Math.round(p.maxHp*k.ev.frac)));p.hp+=h;this.event('heal',{player:id,amount:h});}}
 ...
 /* 카운터 뒤 일시 탭 — 솔로 js/combat.js 와 같은 규칙 (R.opening, docs/design/78).
    아인: 큰 기술 카운터 → 부위 파괴 · 카인: 카운터 → 붙잡기 · 아인·카인이 카운터하면 파티의 류·세라에게 부위 파괴 */
 bigPattern(pat){const OP=R.opening,all=this.stage.patterns||[],mx=Math.max(1,...all.map(q=>Math.max(q.dmg||0,...(q.chain||[]).map(c=>c.dmg||0))));return !!pat&&(pat.rank==='S'||pat.big===true||(pat.dmg||0)>=OP.bigFrac*mx);}
 openFor(q,spec,from){const OP=R.opening;if(!spec||!this.alive(q))return;if(spec.kind==='break'&&!this.boss.parts.some(x=>x.breakable&&!x.broken))return;q.opening={kind:spec.kind,clip:spec.clip||'smash',t:OP.dur,dur:OP.dur,from};this.event('opening',{player:q.id,kind:spec.kind,from});}
 grantOpenings(p){const OP=R.opening;if(!OP)return;const ch=p.character||'ain',spec=(OP.byChar||{})[ch];if(spec&&(!spec.bigOnly||this.bigPattern(this.boss.pattern)))this.openFor(p,spec,p.id);
  if((OP.allyFrom||[]).includes(ch))for(const q of this.players.values())if(q!==p&&(OP.ally||{})[q.character])this.openFor(q,OP.ally[q.character],p.id);}
 useOpening(p){const OP=R.opening,o=p.opening;if(!OP||!o||p.lock>0||this.state!=='fight')return;p.opening=null;p.action=null;p.buffer=null;
  if(o.kind==='break'){let tp=this.boss.parts.find(x=>x.id===p.target);if(!tp||!tp.breakable||tp.broken)tp=this.boss.parts.find(x=>x.breakable&&!x.broken)||tp;if(tp)p.target=tp.id;this.action(p,'opening',OP.breakMult,this.policy().normal,{clip:o.clip,opening:'break'});}
  else this.action(p,'opening',OP.grabMult,this.policy().normal,{clip:o.clip,opening:'grab'});
  this.event('openinguse',{player:p.id,kind:o.kind});}
 action(p,kind,mult,reward,opt={}){const clip=opt.clip||(kind==='counter'?'counter':kind==='ult'?'ult':'smash'),overrides=(R.motion.characterProfiles||{})[p.character||'ain']||{},profile=overrides[clip]||R.motion[kind==='attack'?'light':kind]||R.motion.light,speed=p.stats.aspd/100;p.action={id:++this.serial,kind,clip,part:p.target,elapsed:0,hitAt:profile.hit/speed,duration:profile.duration/speed,cancelAt:profile.cancel/speed,clipHit:((R.motion.clipContactsByChar||{})[p.character||'ain']||{})[clip]||R.motion.clipContacts[clip]||profile.clipHit,mult,reward,opt,resolved:false};p.action.events=opt.ev?SKILL_EVENTS.schedule(opt.ev,p.action):null;p.action.evI=0;p.riposteT=0;p.guard=false;p.regenDelay=R.stamina.delay;const center=QUALITY.partCenter({...this.boss,arena:this.A.id,scale:(this.A.bossScale||1.22)*(this.A.scale||1)},this.boss.parts.find(q=>q.id===p.target)||this.boss.parts[0],this.A.parts3d);p.aim=this.world.angle(p.x,p.y,center.x,center.y);}
 ...
 /* e: 사건 하나(다단의 한 타, 또는 투척의 손 떠남). 없으면 예전처럼 한 번에 전부 */
 impact(p,a,e){a.resolved=true;const first=!e||e.i===0,last=!e||e.i===e.n-1,targets=a.opt.aoe?this.boss.parts:[this.boss.parts.find(q=>q.id===a.part)||this.boss.parts[0]],hits=targets.map(part=>({part,contact:this.contact(p,part,a)})).filter(h=>h.contact);
  if(this.state!=='fight'||!hits.length){if(first){p.whiffs++;p.lastFailure='공격이 닿지 않았습니다. 거리와 방향을 확인하세요.';}this.event('whiff',{player:p.id});return;}
  if(e&&e.kind==='release'){this.event('release',{player:p.id,clip:a.clip,flight:e.flight,part:hits[0].part.id});for(const b of e.blasts)(p.proj||(p.proj=[])).push({t:e.flight+b.delay,a,hits,n:targets.length,b});return;}
  this.strike(p,e?this.slice(a,e.w,first,last):a,hits,targets.length,first);}
 /* 다단 한 타: 배율은 가중치만큼, 자세·반격·잡기는 첫 타에만, 출혈은 마지막 타에만 */
 slice(a,w,first,last){return {...a,mult:a.mult*w,opt:{...a.opt,bleed:last?a.opt.bleed:0,posture:first?a.opt.posture:0,riposte:first?a.opt.riposte:null,opening:first?a.opt.opening:null,evade:first&&a.opt.evade}};}
 detonate(p,pr){if(this.state!=='fight')return;const first=pr.b.i===0,last=pr.b.i===pr.b.n-1;this.event('detonate',{player:p.id,clip:pr.a.clip,part:pr.hits[0].part.id,seq:pr.b.i+1,of:pr.b.n});this.strike(p,this.slice({...pr.a,contact:null},pr.b.w,first,last),pr.hits.map(h=>({...h,contact:null})),pr.n,first);}
 strike(p,a,hits,div,first){const targets={length:div};
  if(a.opt.evade)p.evadeRetaliations++;for(const h of hits){if(this.state!=='fight')break;this.damage(p,h.part,{...a,contact:h.contact},targets.length);}if(!first)return;if(a.opt.opening==='grab'&&this.state==='fight'&&this.boss.state!=='downed'){this.boss.posture+=R.opening.grabPosture;this.boss.state='stagger';this.boss.timer=Math.max(this.boss.timer||0,R.opening.grabHold);this.boss.zone=null;this.event('grab',{player:p.id,hold:R.opening.grabHold});this.checkDown();}if(a.opt.riposte&&!a.opt.followup||a.opt.posture){this.boss.posture+=a.opt.posture||25;this.checkDown();}p.ult=clamp(p.ult+(a.kind==='counter'?18:2),0,100);
 ...
 damage(p,part,a,div){const policy=this.policy(),guard=part.guardedBy?.some(id=>this.boss.parts.some(q=>q.id===id&&!q.broken))?1-part.guardReduce:1,weak=part.broken?policy.exposed:part.weak?1.5:1;
  const pt=a.contact&&a.kind!=='counter'&&!a.opt.riposte?SWING_POINT.point(a.clip,a.contact):null;
  const crit=p.critNext||crypto.randomInt(100000)<(p.stats.critChance||0)*100000;const amount=Math.round((pt?pt.mult:1)*p.stats.atk*(p.attackBuffT>0?1.03:1)*(a.opt.skill?(p.stats.skillMult||1):1)*a.mult*a.reward*weak*guard*(this.boss.state==='downed'?1.5:1)*(crit?(p.stats.critDamage||1.426):1)/div);p.critNext=false;if(a.opt.bleed||crypto.randomInt(100000)<(p.stats.bleedChance||0)*100000){this.boss.bleeds=this.boss.bleeds.filter(d=>d.owner!==p.id);this.boss.bleeds.push({owner:p.id,remaining:p.stats.bleedDuration||3,next:1});}this.boss.hp=Math.max(0,this.boss.hp-amount);p.damage+=amount;p.threat+=amount;this.event('hit',{player:p.id,part:part.id,damage:amount,kind:a.kind,contact:a.contact||null,point:pt,perfect:!!a.opt.perfect,material:QUALITY.material(part.id,this.stage.kind)});
  if(part.breakable&&!part.broken){part.hp=Math.max(0,part.hp-amount*(policy.partMult||1.3)*(a.opt.breakMult||1)*(a.kind==='counter'||a.opt.riposte&&!a.opt.followup?1.6:1)*(p.stats.partMult||1)-(a.opt.opening==='break'?Math.round((part.hpMax||0)*R.opening.breakFrac):0));if(part.hp===0){part.broken=true;p.breaks++;const burst=Math.round(p.stats.atk*policy.breakBurst);this.boss.hp=Math.max(0,this.boss.hp-burst);p.damage+=burst;this.boss.posture+=40+(part.onBreak?.posture||0);this.boss.state='stagger';this.boss.timer=.8;this.boss.zone=null;if(part.onBreak?.slow)this.boss.slow=Math.min(this.boss.slow,part.onBreak.slow);if(part.onBreak?.zoneScale)this.boss.zoneScale=Math.min(this.boss.zoneScale,part.onBreak.zoneScale);this.event('break',{player:p.id,part:part.id,name:part.name});this.checkDown();}}
 ...
 /* 경직은 솔로(js/combat.js)와 같은 규칙표 R.stagger 를 쓴다 — 맞은 세기에 따라
    굳는 시간과 밀리는 거리가 다르다. opt.from 이 있으면 그 반대로 밀린다. */
 hurt(p,damage,reason,opt){const o=opt||{};
  const dealt=Math.round(damage*(p.buffT>0?1-(p.buffReduce??.5):1)*(1-Math.min(.25,(p.stats.defense||0)/((p.stats.defense||0)+5000))));
  p.hp=Math.max(0,p.hp-dealt);p.lastFailure=reason;
  const SG=R.stagger||{},tier=o.guarded?'guard':(dealt>=p.maxHp*(SG.heavyAt||.12)?'heavy':'light'),sg=SG[tier]||{};
  p.lock=sg.lock??.28;
  if(sg.push&&o.from)this.world.knock(p,p.x-o.from.x,p.y-o.from.y,sg.push,sg.dur||.18);
  p.action=null;p.buffer=null;p.reviveHeld=false;
  this.event('hurt',{player:p.id,damage:Math.round(damage),reason,tier});
  if(!p.hp){p.deaths++;p.downT=20;p.guard=false;p.axes={x:0,y:0};p.rollT=0;p.kbT=0;this.event('playerDown',{player:p.id});}}
 ...
 bossStep(dt){const b=this.boss;if(b.state==='idle'){const p=this.target();if(!p)return;this.world.bossThink(b,p,dt,{...this.L.ai[this.phase],speed:this.L.ai[this.phase].speed*b.slow},false);b.aim=this.world.angle(b.x,b.y,p.x,p.y);b.timer-=dt;
  ...
  }else if(b.state==='telegraph'){b.tele=Math.max(0,b.tele-dt);this.director.advance(this.windup(b.pattern,1-b.tele/b.teleDur,b.teleDur));if(b.tele<=1e-8){for(const p of this.players.values()){if(!this.alive(p))continue;const inside=this.inZone(p);if(!inside||p.dodgeT>0){if(p.dodgeThreat===b.attackId&&this.time-p.dodgeAt<=.48){const perfect=this.time-p.dodgeAt<=(R.dodge.perfect||0);p.riposteT=.85+(perfect?(R.dodge.perfectRiposte||0):0);p.riposteKind='evade';p.evades++;if(perfect){p.perfectDodges=(p.perfectDodges||0)+1;p.st=Math.min(R.stamina.max,p.st+R.stamina.dodge);}if(p.stats.evadeBuff)p.moveBuffT=3;this.event('evade',{player:p.id,perfect});}continue;}const guarded=p.guard&&p.st>0&&b.pattern.unblockable!==true;if(guarded){p.st=Math.max(0,p.st-(b.pattern.guardCost||20));p.riposteT=.8;p.riposteKind='guard';}this.hurt(p,b.pattern.dmg*(guarded?.3:1),b.pattern.name,{guarded,from:{x:b.x,y:b.y}});if(!guarded&&this.A.id!=='tutorial'){p.bleedT=6*(p.stats.statusDuration||1);p.bleedTick=1;}}const nx=this.nextBeat(b.beatI+1);
 ...
 tick(dt=.01){if(this.state==='clear'&&this.result.rewardStatus==='pending'){this.rewardRetry-=dt;if(this.rewardRetry<=0)this.settleRewards();}if(!['explore','fight','transition'].includes(this.state)||![...this.players.values()].some(p=>p.connected))return;this.time+=dt;
  ...
  const reviving=new Set();for(const p of this.players.values()){
   ...
   for(const key of ['dodgeT','dodgeCd','lock','regenDelay','riposteT','comboT','buffT','hazardCd','itemCd','attackBuffT','moveBuffT'])p[key]=Math.max(0,p[key]-dt);if(p.opening){p.opening.t-=dt;if(p.opening.t<=0){p.opening=null;this.event('openingend',{player:p.id});}}p.cds=p.cds.map(t=>Math.max(0,t-dt));p.threat*=Math.exp(-dt*.12);
   ...
   if(p.action){const a=p.action;a.elapsed+=dt;if(a.events){while(p.action===a&&a.evI<a.events.length&&a.elapsed>=a.events[a.evI].t)this.impact(p,a,a.events[a.evI++]);}else if(!a.resolved&&a.elapsed>=a.hitAt)this.impact(p,a);if(a.elapsed>=a.duration&&p.action===a)p.action=null;}
   if(p.proj?.length){for(const q of p.proj)q.t-=dt;const due=p.proj.filter(q=>q.t<=1e-8);if(due.length){p.proj=p.proj.filter(q=>q.t>1e-8);for(const q of due)this.detonate(p,q);}}
  ...
  if(this.state==='explore'){
   ...
   for(const p of this.players.values()){if(!this.alive(p))continue;if(p.hazardCd<=0&&p.dodgeT<=0){const h=this.expedition.hazards.find(h=>this.expedition.hazardPhase(h)==='active'&&this.world.dist(p.x,p.y,h.x,h.y)<h.r);if(h){p.hazardCd=.9;this.hurt(p,p.maxHp*(h.damage||.08),'분출',{from:{x:h.x,y:h.y}});}}
   ...
   if(this.state==='fight'&&this.arenaHz&&this.arenaHz.length)for(const p of this.players.values()){
    ...
    if(h){p.hazardCd=.9;this.hurt(p,p.maxHp*h.damage,(this.arenaFx&&this.arenaFx.reason)||'아레나가 변했다. 위험 구역을 피해라.',{from:{x:h.x,y:h.y}});}}
