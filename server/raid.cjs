...
class Raid{
 ...
 input(id,msg){const p=this.players.get(id);if(!p||!p.connected||!this.alive(p)||!['explore','fight'].includes(this.state))return;
  ...
  if(msg.type==='attack'&&this.canCounter(p)){const perfect=this.boss.tele<=.04+1e-8;p.counters++;if(p.stats.counterBuff)p.attackBuffT=5;p.perfect+=+perfect;p.guard=false;this.boss.state='stagger';this.boss.timer=.8;this.boss.zone=null;this.boss.posture+=30;this.action(p,'counter',1,(perfect?R.counter.perfectMult:R.counter.mult)*(p.stats.counterMult||1),{perfect});this.event('counter',{player:id,perfect});this.checkDown();return;}
  if(msg.type==='attack'){const tier=p.comboT>0?p.combo%4:0;this.action(p,'attack',R.combo.mults[tier],p.riposteT>0?(p.riposteKind==='guard'?1.5:2.2):this.policy().normal,{riposte:p.riposteT>0,evade:p.riposteT>0&&p.riposteKind==='evade',clip:'attack'+(tier%3+1)});p.combo=tier+1;p.comboT=1.3;return;}
  ...
  if(msg.type==='skill'){if(!Number.isInteger(msg.index)||msg.index<0||msg.index>3)return;const k=p.skills[msg.index];if(!k||p.cds[msg.index]>0||p.st<k.st)return;
   ...
   else this.action(p,'smash',k.mult,this.policy().skill,{aoe:k.aoe,skill:true,posture:k.posture,bleed:k.bleed});return;}
 ...
 impact(p,a){a.resolved=true;if(this.state!=='fight'||!this.nearBoss(p)||this.world.angDiff(p.aim,this.world.angle(p.x,p.y,this.boss.x,this.boss.y))>this.L.player.cone){p.whiffs++;p.lastFailure='공격이 닿지 않았습니다. 거리와 방향을 확인하세요.';this.event('whiff',{player:p.id});return;}
  if(a.opt.evade)p.evadeRetaliations++;const targets=a.opt.aoe?this.boss.parts:[this.boss.parts.find(q=>q.id===a.part)||this.boss.parts[0]];for(const part of targets){if(this.state!=='fight')break;this.damage(p,part,a,a.opt.aoe?targets.length:1);}if(a.opt.riposte||a.opt.posture){this.boss.posture+=a.opt.posture||25;this.checkDown();}p.ult=clamp(p.ult+(a.kind==='counter'?18:2),0,100);}
 ...
 tick(dt=.01){if(this.state==='clear'&&this.result.rewardStatus==='pending'){this.rewardRetry-=dt;if(this.rewardRetry<=0)this.settleRewards();}if(!['explore','fight','transition'].includes(this.state)||![...this.players.values()].some(p=>p.connected))return;this.time+=dt;
  ...
  }else {for(const dot of this.boss.bleeds){dot.remaining-=dt;dot.next-=dt;const owner=this.players.get(dot.owner);if(dot.next<=0&&owner){dot.next=1;const damage=Math.round(owner.stats.atk*.03*(owner.stats.bleedMult||1));this.boss.hp=Math.max(0,this.boss.hp-damage);owner.damage+=damage;this.event('hit',{player:owner.id,part:'bleed',damage,kind:'bleed'});if(this.boss.hp===0){this.phaseClear();break;}}}this.boss.bleeds=this.boss.bleeds.filter(d=>d.remaining>0);if(this.state==='fight')this.bossStep(dt);}
 ...
 snapshot(){const b=this.boss;return {id:this.id,level:this.L.id,state:this.state,time:this.time,phase:this.phase,phases:this.A.stages.length,boss:{...b,def:undefined,beats:undefined,pattern:b.pattern&&{name:b.pattern.name,icon:b.pattern.icon,counterable:b.pattern.counterable!==false,hold:b.pattern.hold||null,beat:b.pattern.beat+1,beats:b.pattern.beats,last:!!b.pattern.final},
  windup:b.state==='telegraph'&&b.teleDur?this.windup(b.pattern,1-b.tele/b.teleDur,b.teleDur):1,name:this.stage.name,window:this.stage.counterWindow||.18},players:[...this.players.values()].map(({buffer,lastInput,dodgeThreat,threat,axes,skills,ultSkill,...p})=>({...p,kit:skills.map(k=>({name:k.name,lv:k.lv,br:k.br,st:k.st})),ultName:ultSkill.name,ultLv:ultSkill.lv})),expedition:this.expedition.snapshot(),hazards:this.expedition.hazards.map(h=>({...h,phase:this.expedition.hazardPhase(h)})),events:this.events.slice(-20),training:this.training||null,result:this.result||null};}
}
module.exports={Raid};
