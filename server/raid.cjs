...
class Raid{
 constructor(levelId,members,onClear){
  this.L=C.levels[levelId];this.A=C.arenas[this.L.arena];this.id=crypto.randomUUID();this.time=0;this.state='explore';this.phase=0;this.events=[];this.serial=0;this.onClear=onClear;this.scale=1+.65*(members.length-1);this.players=new Map();
 ...
 input(id,msg){const p=this.players.get(id);if(!p||!p.connected||!this.alive(p)||!['explore','fight'].includes(this.state))return;
  ...
  if(msg.type==='guard'){if(!msg.on){p.guard=false;return;}if(!p.action&&p.lock<=0&&p.dodgeT<=0)p.guard=true;return;}
  if(msg.type==='revive'){p.reviveHeld=msg.on===true;return;}
  if(msg.type==='interact'){if(this.state==='explore')this.interact(p);return;}
  if(!['attack','smash','dodge','skill','ult'].includes(msg.type))return;
  if(p.lock>0||p.dodgeT>0)return;
  if(p.action){const late=p.action.elapsed>=p.action.cancelAt;if((msg.type==='dodge'||msg.type==='attack'&&this.canCounter(p))&&late)p.action=null;else{if(p.action.duration-p.action.elapsed<=R.motion.buffer)p.buffer={...msg,expires:this.time+R.motion.buffer};return;}}
  if(msg.type==='dodge'){this.dodge(p,R.stamina.dodge);return;}
  if(msg.type==='attack'&&this.canCounter(p)){const perfect=this.boss.tele<=.04+1e-8;p.counters++;p.perfect+=+perfect;p.guard=false;this.boss.state='stagger';this.boss.timer=.8;this.boss.zone=null;this.boss.posture+=30;this.action(p,'counter',1,perfect?R.counter.perfectMult:R.counter.mult,{perfect});this.event('counter',{player:id,perfect});this.checkDown();return;}
  if(msg.type==='attack'){const tier=p.comboT>0?p.combo%4:0;this.action(p,'attack',R.combo.mults[tier],p.riposteT>0?(p.riposteKind==='guard'?1.5:2.2):this.policy().normal,{riposte:p.riposteT>0,clip:'attack'+(tier%3+1)});p.combo=tier+1;p.comboT=1.3;return;}
  if(msg.type==='smash'){const tier=p.comboT>0?Math.max(0,p.combo-1):0;if(p.st<R.combo.smashSt[tier])return;p.st-=R.combo.smashSt[tier];this.action(p,'smash',R.combo.smash[tier],p.riposteT>0?(p.riposteKind==='guard'?1.5:2.2):this.policy().normal,{posture:R.combo.smashPosture[tier],riposte:p.riposteT>0});p.combo=0;return;}
  if(msg.type==='skill'){if(!Number.isInteger(msg.index)||msg.index<0||msg.index>3)return;const k=C.skills.ain[msg.index];if(p.cds[msg.index]>0||p.st<k.st)return;
   if(k.dodge&&p.dodgeCd>0)return;p.st-=k.st;p.cds[msg.index]=k.cd;p.regenDelay=R.stamina.delay;if(k.dodge){this.dodge(p,0);p.critNext=true;}else if(k.buff)p.buffT=k.buff.dur;else this.action(p,'smash',k.mult,this.policy().skill,{aoe:k.aoe});return;}
  if(msg.type==='ult'&&p.ult>=100){p.ult=0;this.action(p,'ult',C.skills.ainUlt.mult,1);}
 }
 ...
 impact(p,a){a.resolved=true;if(this.state!=='fight'||!this.nearBoss(p)||this.world.angDiff(p.aim,this.world.angle(p.x,p.y,this.boss.x,this.boss.y))>this.L.player.cone){this.event('whiff',{player:p.id});return;}
  const targets=a.opt.aoe?this.boss.parts:[this.boss.parts.find(q=>q.id===a.part)||this.boss.parts[0]];for(const part of targets){if(this.state!=='fight')break;this.damage(p,part,a,a.opt.aoe?targets.length:1);}if(a.opt.riposte||a.opt.posture){this.boss.posture+=a.opt.posture||25;this.checkDown();}p.ult=clamp(p.ult+(a.kind==='counter'?18:2),0,100);}
 damage(p,part,a,div){const policy=this.policy(),guard=part.guardedBy?.some(id=>this.boss.parts.some(q=>q.id===id&&!q.broken))?1-part.guardReduce:1,weak=part.broken?policy.exposed:part.weak?1.5:1;
  const amount=Math.round(p.stats.atk*a.mult*a.reward*weak*guard*(this.boss.state==='downed'?1.5:1)*(p.critNext?1.426:1)/div);p.critNext=false;this.boss.hp=Math.max(0,this.boss.hp-amount);p.damage+=amount;p.threat+=amount;this.event('hit',{player:p.id,part:part.id,damage:amount,kind:a.kind});
  if(part.breakable&&!part.broken){part.hp=Math.max(0,part.hp-amount*(policy.partMult||1.3)*(a.kind==='counter'||a.opt.riposte?1.6:1));if(part.hp===0){part.broken=true;p.breaks++;const burst=Math.round(p.stats.atk*policy.breakBurst);this.boss.hp=Math.max(0,this.boss.hp-burst);p.damage+=burst;this.boss.posture+=40+(part.onBreak?.posture||0);this.boss.state='stagger';this.boss.timer=.8;this.boss.zone=null;if(part.onBreak?.slow)this.boss.slow=Math.min(this.boss.slow,part.onBreak.slow);if(part.onBreak?.zoneScale)this.boss.zoneScale=Math.min(this.boss.zoneScale,part.onBreak.zoneScale);this.event('break',{player:p.id,part:part.id,name:part.name});this.checkDown();}}
  if(this.boss.hp<=0)this.phaseClear();
 }
 checkDown(){if(this.boss.hp<=0)return;const breakable=this.boss.parts.filter(p=>p.breakable);if(this.boss.posture>=100||this.stage.allBrokenDown&&breakable.length&&breakable.every(p=>p.broken)&&!this.boss.allDown){this.boss.allDown=true;this.boss.state='downed';this.boss.timer=5;this.boss.zone=null;this.boss.posture=0;this.event('down');}}
 hurt(p,damage,reason){p.hp=Math.max(0,p.hp-Math.round(damage*(p.buffT>0?.5:1)*(1-Math.min(.25,(p.stats.defense||0)/((p.stats.defense||0)+5000)))));p.lock=.28;p.action=null;p.buffer=null;p.reviveHeld=false;this.event('hurt',{player:p.id,damage:Math.round(damage),reason});if(!p.hp){p.downT=20;p.guard=false;p.axes={x:0,y:0};p.rollT=0;this.event('playerDown',{player:p.id});}}
 ...
 phaseClear(){this.boss.zone=null;this.event('phaseClear',{phase:this.phase});if(this.phase+1<this.A.stages.length){this.state='transition';this.transition=1.4;}else{this.state='clear';this.event('clear');this.result={run:this.id,arena:this.A.id,time:this.time,...this.rewards(),rewardStatus:'pending',players:[...this.players.values()].map(p=>({id:p.id,name:p.name,damage:p.damage,counters:p.counters,perfect:p.perfect,breaks:p.breaks}))};this.settleRewards();}}
 ...
 bossStep(dt){const b=this.boss;if(b.state==='idle'){const p=this.target();if(!p)return;this.world.bossThink(b,p,dt,{...this.L.ai[this.phase],speed:this.L.ai[this.phase].speed*b.slow},false);b.aim=this.world.angle(b.x,b.y,p.x,p.y);b.timer-=dt;
   if(b.timer<=0&&this.nearBoss(p,this.L.ai[this.phase].start)){this.focus.x=p.x;this.focus.y=p.y;const available=this.stage.patterns.filter(a=>!(a.disabledBy||[]).some(id=>b.parts.some(q=>q.id===id&&q.broken)));if(!available.length)return;const pat=this.director.pick(available,b.patternIndex++),spec={...this.L.zones[pat.name]};for(const k of ['r','w','len'])if(spec[k])spec[k]*=b.zoneScale;const plan=this.director.start(pat,spec);b.zone=plan.zone;b.aim=plan.angle;b.target=p.id;b.pattern=pat;b.state='telegraph';b.teleDur=pat.tele+b.parts.filter(q=>q.broken).reduce((n,q)=>n+(q.onBreak?.telePlus||0),0);b.tele=b.teleDur;b.attackId=++this.serial;this.event('telegraph',{target:p.id,pattern:pat.name});}
  }else if(b.state==='telegraph'){b.tele=Math.max(0,b.tele-dt);this.director.advance(1-b.tele/b.teleDur);if(b.tele<=1e-8){for(const p of this.players.values()){if(!this.alive(p))continue;const inside=this.inZone(p);if(!inside||p.dodgeT>0){if(p.dodgeThreat===b.attackId&&this.time-p.dodgeAt<=.48){p.riposteT=.85;p.riposteKind='evade';p.evades++;this.event('evade',{player:p.id});}continue;}const guarded=p.guard&&p.st>0&&b.pattern.unblockable!==true;if(guarded){p.st=Math.max(0,p.st-(b.pattern.guardCost||20));p.riposteT=.8;p.riposteKind='guard';}this.hurt(p,b.pattern.dmg*(guarded?.3:1),b.pattern.name);}b.state='recover';b.recoveryDur=b.pattern.recovery||.8;b.recovery=b.recoveryDur;this.event('swing');}}
 ...
 tick(dt=.01){if(this.state==='clear'&&this.result.rewardStatus==='pending'){this.rewardRetry-=dt;if(this.rewardRetry<=0)this.settleRewards();}if(!['explore','fight','transition'].includes(this.state)||![...this.players.values()].some(p=>p.connected))return;this.time+=dt;
  ...
  const reviving=new Set();for(const p of this.players.values()){
   ...
   for(const key of ['dodgeT','dodgeCd','lock','regenDelay','riposteT','comboT','buffT','hazardCd'])p[key]=Math.max(0,p[key]-dt);p.cds=p.cds.map(t=>Math.max(0,t-dt));p.threat*=Math.exp(-dt*.12);
   ...
   if(p.buffer){if(p.buffer.expires<this.time)p.buffer=null;else if(!p.action){const msg=p.buffer;p.buffer=null;this.input(p.id,msg);}}
   if(!['explore','fight'].includes(this.state))break;
   p.lockT=p.action||p.guard||p.lock>0||p.reviveHeld?1:0;this.world.movePlayer(p,p.axes.x,p.axes.y,dt,this.L.player.speed);
 ...
 retry(){if(this.state!=='wiped')return;this.phase=0;this.setupBoss();this.state='explore';const spawn=this.expedition.checkpoint()||this.world.marks('S')[0];let i=0;for(const [id,p]of this.players){const fresh=this.player(p,spawn,i++);fresh.connected=p.connected;this.players.set(id,fresh);}this.world.setSolid(this.gate.cx,this.gate.cy,!this.expedition.ready());this.event('retry');}
 snapshot(){const b=this.boss;return {id:this.id,level:this.L.id,state:this.state,time:this.time,phase:this.phase,phases:this.A.stages.length,boss:{...b,pattern:b.pattern&&{name:b.pattern.name,icon:b.pattern.icon,counterable:b.pattern.counterable!==false},name:this.stage.name,window:this.stage.counterWindow||.18},players:[...this.players.values()].map(({buffer,lastInput,dodgeThreat,threat,axes,...p})=>p),expedition:this.expedition.snapshot(),hazards:this.expedition.hazards.map(h=>({...h,phase:this.expedition.hazardPhase(h)})),events:this.events.slice(-20),result:this.result||null};}
}
module.exports={Raid};
