...
function createPartyServer(options={}){
 ...
 function roomState(room){return {type:'state',protocol:2,code:room.code,leader:room.leader,level:room.level,public:room.public,purpose:room.purpose||'first',training:!!room.training,members:[...room.members.values()].map(({id,name,character,ready,connected})=>({id,name,character:character||'ain',ready,connected})),raid:room.raid?.snapshot()||null};}
