...
function createPartyServer(options={}){
 ...
 function roomState(room){return {type:'state',protocol:2,code:room.code,leader:room.leader,level:room.level,public:room.public,purpose:room.purpose||'first',training:!!room.training,members:[...room.members.values()].map(({id,name,ready,connected})=>({id,name,ready,connected})),raid:room.raid?.snapshot()||null};}
 ...
 async function command(ws,msg){
  ...
  if(msg.type==='character'){const p=store.chooseName(id,msg.name);profileUpdate(p);enterOffice(id);rpg.update(id);sendBoard();return;}
