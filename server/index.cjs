...
function createPartyServer(options={}){
 ...
 function lobbyInfo(id){try{const p=store.public(id);return {level:p.level,power:POWER(p.stats)};}catch{return {level:1,power:0};}}
 function roomState(room){const lobby=!room.raid;return {type:'state',protocol:2,code:room.code,leader:room.leader,level:room.level,public:room.public,purpose:room.purpose||'first',training:!!room.training,members:[...room.members.values()].map(({id,name,character,ready,connected})=>({id,name,character:character||'ain',ready,connected,...(lobby?lobbyInfo(id):{})})),raid:room.raid?.snapshot()||null};}
 ...
 function board(){return {type:'board',online:sessions.size,capacity:maxPlayers,office:[...sessions.keys()].filter(id=>!current(id)?.raid).length,rooms:[...rooms.values()].filter(r=>r.public&&!r.raid&&r.members.size<4).map(r=>({code:r.code,level:r.level,purpose:r.purpose||'first',count:r.members.size,leader:r.members.get(r.leader)?.name||'아인'}))};}
 ...
 function join(room,profile){if(room.training)throw Error('개인 훈련에는 참가할 수 없습니다.');if(!store.canEnter(profile.id,room.level))throw Error('선행 의뢰 보수를 수령해 지역을 해금하세요.');if(room.raid)throw Error('출격 중인 방은 새로 참가할 수 없습니다.');if(room.members.size>=4)throw Error('파티가 가득 찼습니다.');if(current(profile.id))throw Error('현재 파티에서 먼저 나가 주세요.');room.members.set(profile.id,{id:profile.id,name:profile.name,character:profile.character||'ain',ready:false,connected:true});memberships.set(profile.id,room.code);broadcast(room);sendHistory(profile.id);sendBoard();}
 ...
 async function command(ws,msg){
  ...
  if(msg.type==='create'){requireOffice(id);if(room)throw Error('현재 파티에서 먼저 나가 주세요.');if(rooms.size>=maxRooms)throw Error('서버가 가득 찼습니다.');const level=Object.hasOwn(C.levels,msg.level)?msg.level:'d01';if(!store.canEnter(id,level))throw Error('선행 의뢰 보수를 수령해 지역을 해금하세요.');let code;do{code=crypto.randomBytes(5).toString('hex').toUpperCase();}while(rooms.has(code));const next={code,leader:id,level,public:msg.public!==false,purpose:['practice','first','repeat'].includes(msg.purpose)?msg.purpose:'first',members:new Map(),raid:null};rooms.set(code,next);join(next,store.public(id));return;}
