...
function createPartyServer(options={}){
 ...
 const POWER=st=>Math.round((st?.atk||0)*6+(st?.hp||0)*0.3);   /* 전투력: 공격 6 + 체력 0.3 */
 /* 대기 중인 방에서만 읽는다 — 시트 03 의 카드에는 Lv·전투력 아래 5줄 등급표가 있다. */
 const NOGRADE={counter:'C',break:'C',refine:'C',drop:'C',craft:'C'};
 function lobbyInfo(id){try{const p=store.public(id);
  return {level:p.level,power:POWER(p.stats),grades:store.techGrades(id)};}
  catch{return {level:1,power:0,grades:{...NOGRADE}};}}
 function roomState(room){const lobby=!room.raid;return {type:'state',protocol:2,code:room.code,leader:room.leader,level:room.level,public:room.public,purpose:room.purpose||'first',training:!!room.training,minPower:room.minPower||0,voice:!!room.voice,members:[...room.members.values()].map(({id,name,character,ready,connected})=>({id,name,character:character||'ain',ready,connected,...(lobby?lobbyInfo(id):{})})),raid:room.raid?.snapshot()||null};}
 ...
 function board(){return {type:'board',online:sessions.size,capacity:maxPlayers,office:[...sessions.keys()].filter(id=>!current(id)?.raid).length,rooms:[...rooms.values()].filter(r=>r.public&&!r.raid).map(r=>({code:r.code,level:r.level,purpose:r.purpose||'first',count:r.members.size,leader:r.members.get(r.leader)?.name||'아인',chars:[...r.members.values()].map(m=>m.character||'ain'),minPower:r.minPower||0,voice:!!r.voice}))};}
 ...
 function guildState(id){const g=store.guildDetails(id);return g?{...g,members:g.members.map(p=>({...p,online:sessions.has(p.id)}))}:null;}
 ...
 function attach(ws,result){
  ...
  store.checkBan(result.profile.id);store.deliverRewards(result.profile.id);result.profile=store.public(result.profile.id);const {profile,token}=result;if(!sessions.has(profile.id)&&sessions.size>=maxPlayers)throw Error('서버가 가득 찼습니다. 잠시 후 다시 접속하세요.');
 ...
 async function account(ws,msg){
  ...
  if(!['register','login','recover'].includes(msg.mode))throw Error('계정 요청을 확인하세요.');
  ...
  pendingAuth++;
  try{const result=msg.mode==='register'?await store.register(msg.username,msg.password,msg.name,upgrade||undefined):msg.mode==='recover'?await store.recover(msg.username,msg.code,msg.password):await store.authenticate(msg.username,msg.password);attach(ws,result);}finally{pendingAuth--;}
 }
 const rpg=createRpgCommands({store,sessions,send,current,requireOffice,profileUpdate,unready,broadcast,sendBoard,sendGuild,sendHistory,refreshGuild,join,rooms,
  async passwordChange(ws,msg){if(pendingAuth>=4)throw Error('계정 요청이 많습니다.');pendingAuth++;try{const result=await store.changePassword(ws.playerId,msg.current,msg.password);send(ws,{type:'credentials',...result});}finally{pendingAuth--;}},
  rememberWhisper(message){for(const id of new Set([message.player,message.target])){const key='whisper:'+id,list=history(key);list.push(message);if(list.length>50)list.shift();histories.set(key,list);}},
  evidence(id,target){const g=store.guild(id),room=current(id);return [...history('world'),...history('guild:'+g?.id),...history('party:'+room?.code),...history('whisper:'+id)].filter(m=>m.player===target).slice(-5).map(m=>m.name+': '+m.text).join(' | ');}
 });
 async function command(ws,msg){
  ...
  if(['guildCreate','guildJoin','guildLeave'].includes(msg.type)){
   ...
   refreshGuild(id);sendGuild(id);sendHistory(id);rpg.update(id);return;
  }
  if(msg.type==='chat'){if(store.sanction(id).mute_until>Date.now())throw Error('채팅이 제한된 계정입니다.');
   ...
   ws.lastChat=Date.now();const message={id:++chatSerial,channel:msg.channel,name:store.public(id).name,player:id,text,created:Date.now()};const messages=history(key);messages.push(message);if(messages.length>50)messages.shift();histories.set(key,messages);
   const payload=JSON.stringify({type:'chat',message});for(const pid of ids){const peer=sessions.get(pid);if(peer&&!store.blocked(pid,id))send(peer,payload);}return;
  }
  ...
  /* 모집 목록 새로 고침 (시트 03 필터 바) — 방 생성·합류·퇴장 때 자동으로도 나가지만
     사용자가 직접 다시 받아 볼 수 있어야 한다. 1초에 한 번으로 묶는다. */
  if(msg.type==='board'){requireOffice(id);if(Date.now()-(ws.lastBoard||0)<1000)return;ws.lastBoard=Date.now();send(ws,board());return;}
  if(msg.type==='create'){requireOffice(id);if(room)throw Error('현재 파티에서 먼저 나가 주세요.');if(rooms.size>=maxRooms)throw Error('서버가 가득 찼습니다.');const level=Object.hasOwn(C.levels,msg.level)?msg.level:'d01';if(!store.canEnter(id,level))throw Error('선행 의뢰 보수를 수령해 지역을 해금하세요.');let code;do{code=crypto.randomBytes(5).toString('hex').toUpperCase();}while(rooms.has(code));const minPower=Number.isSafeInteger(msg.minPower)&&msg.minPower>0?Math.min(999999,msg.minPower):0;const next={code,leader:id,level,public:msg.public!==false,purpose:['practice','first','repeat'].includes(msg.purpose)?msg.purpose:'first',minPower,voice:msg.voice===true,members:new Map(),raid:null};rooms.set(code,next);join(next,store.public(id));return;}
  ...
  if(msg.type==='training'){requireOffice(id);if(room)throw Error('파티에서 나온 뒤 훈련하세요.');if(rooms.size>=maxRooms)throw Error('서버가 가득 찼습니다.');const phase=Number(msg.phase),pattern=Number(msg.pattern);if(!Number.isInteger(phase)||!Number.isInteger(pattern)||!C.arenas.tutorial.stages[phase]?.patterns[pattern])throw Error('훈련 패턴을 선택하세요.');const code=crypto.randomBytes(5).toString('hex').toUpperCase(),m={id,name:store.get(id).name,ready:true,connected:true},next={code,leader:id,level:'d01',public:false,training:true,purpose:'practice',members:new Map([[id,m]])};const raid=new Raid('d01',[{...m,stats:store.stats(id),quickslots:store.get(id).quickslots,skills:store.skillsFor(id),character:store.get(id).character}],r=>{r.result.training=true;});raid.training={phase,pattern};raid.phase=phase;raid.setupBoss();raid.startFight();const player=raid.players.get(id);player.x=raid.boss.x-120;player.y=raid.boss.y;next.raid=raid;rooms.set(code,next);memberships.set(id,code);broadcast(next);sendBoard();return;}
  if(msg.type==='leave'){leave(id,true);send(ws,{type:'left'});enterOffice(id);sendBoard();return;}
  ...
  if(msg.type==='retry'){if(room.raid?.state!=='wiped'&&!(room.training&&room.raid?.state==='clear'))throw Error('전멸 후 재도전할 수 있습니다.');room.raid.retry();broadcast(room);return;}
  if(msg.type==='lobby'){if(room.training){leave(id,true);send(ws,{type:'left'});enterOffice(id);sendBoard();return;}if(!['clear','wiped'].includes(room.raid?.state))throw Error('전투가 끝난 후 인력사무소로 돌아갑니다.');if(room.raid.result?.rewardStatus==='pending')throw Error('보상 저장을 재시도하고 있습니다. 잠시 후 다시 시도하세요.');room.raid=null;for(const m of room.members.values()){m.ready=false;enterOffice(m.id);}broadcast(room);sendBoard();return;}
