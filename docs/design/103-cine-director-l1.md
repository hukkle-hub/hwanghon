...
## 1-4. 전투 피드백 HUD v05 검수 (GPT 패치, 2026-09-26)

v05(피해 숫자 16/13 px·0.62 초, 동시 표시 PC 9·모바일 6, 카운터 배너 32 px, 콤보·상태선 축소)를 적용하고 PC 1280×720·모바일 915×412 에서 쟀다. 판정 뒤 시간을 얼려(0.02 배속) 숫자 16 개를 넣고 DOM 을 셌다.

![위 PC · 아래 모바일: 완벽 카운터 배너 + 숫자 / 16 연타 폭주(상한)](../img/105-feedback-check.jpg)

| 항목 | PC | 모바일 |
|---|---|---|
| 숫자 상한 | 16 개 → **8 개** 표시(≤9), 16·21 px | 16 개 → **6 개**(=상한), 13·17 px |
| 카운터 배너가 가리는 것 | 캐릭터 머리·가슴, 보스 핵·머리 모두 밖 | **보스 가슴(핵 표식) 위에 걸침** — 배너 y 124~202 px, 보스 핵 y≈185 |
| 콤보 | 우측 중앙, 안 가림 | 안 가림 |
| 상태선 | 표시 없음(상태 이상 없던 장면) | 같음 |
| 보스 바 | 안 가림 | 보스가 화면 위로 붙는 순간 머리와 겹침(카메라가 낮을 때) |
| 안내 한 줄(guide) | 하단 중앙 | **화면 중앙(캐릭터 상체 위)** — 옛 규칙 `html.mobile .guide{bottom:126px}`(specificity 0,2,1)가 GPT 의 `.cinema-hud-v1 .guide{bottom:5px}`(0,2,0)를 이긴다 |
