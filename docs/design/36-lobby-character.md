...
## 12. 인벤토리 종이인형 — 실제로 입은 모습

![인벤토리](../img/36-inventory.png)

장비 화면인데 종이인형이 **원화**(`art/full-*.webp`)였다. 무엇을 끼워도 그림은
그대로였고, 실제 모습을 보려면 「3D 외형」 단추로 `viewer.html` iframe 을 띄워야 했다.

이제 `#doll` 자체가 `data-portrait="me-body"` 다. 들어가면 **입은 그대로** 보인다.
(위 그림은 카인 — 대검과 방어구가 그대로 붙어 있다.)

- **캐릭터를 바꾸면** 먼저 원화로 바로 반응하고, 3D 가 구워지면 그 위에 덮는다.
  굽는 데 1~2초 걸리므로 그동안 빈 칸을 두지 않는다
- **장비를 갈아 끼우면** 종이인형도 따라간다. 열쇠가 그대로면 캐시라 공짜다
- 「3D 외형」 iframe 단추는 **그대로 뒀다** — 돌려 보고 확대하는 것은 여전히 그쪽이다

## 13. 붙은 자리 정리

| 화면 | 자리 | 모양 |
|---|---|---|
| 로비 | 배경의 인물 위 (실시간) · 왼쪽 위 얼굴 | 실시간 3D · 얼굴 |
| 프로필 | 신분증 초상 | 얼굴 |
| 인벤토리 | 종이인형 | 전신 |
| 파티 모집 | **내 칸만** | 전신 |
| 결과 | 왼쪽 큰 아트 | 전신 |

전부 `data-portrait` 한 줄로 붙는다. 실패하면 원화가 그대로 남는다.
**214/214**, 22 화면 952px.
