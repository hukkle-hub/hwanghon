...
* **exec(13.8 m/s)** — 기본 공격(32~42)에 한참 못 미친다. **시간 재배분으로는 못 고친다.**
  소스의 최고속이 t=0.138 인데 엔진 판정은 0.58 이라, 워프가 읽는 소스 구간이
  [0, 0.488] 로 잘려 뒤쪽 절반이 버려진다 (최고속은 20.7 로 오르지만 체간 34° → 10°,
  정지 67% → 88%, 처형이 표적에 못 닿는다). 해 보고 되돌렸다 — `tools/3d/punch.py`
  주석에 기록. **소스를 갈아야 하는 건이다.**
* `punch.py` 는 **멱등하지 않다.** 이미 구운 GLB 에 다시 돌리면 이중으로 워프된다
  (실제로 그렇게 깨뜨렸다). 새 클립 하나만 넣을 때는 `--only` 로 범위를 좁힌다.
