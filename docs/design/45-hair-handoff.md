...
### 3.4 올리기 전에 스스로 돌려 보실 수 있습니다

계약을 검사기로 만들어 뒀습니다. 후보를 굽고 나서 한 줄 돌리면 됩니다.

```
node tools/3d/check-hair-contract.mjs art/3d/base/ain_modular_rig_candidate.glb
```

§3.1~§3.2 를 네 항목으로 나눠 봅니다 — **분리**(`js/hair.js` 가 찾는가) ·
**독점**(그 재질을 살·옷이 같이 쓰지 않는가) · **스킨**(Head 뼈를 가진 리그에 물렸는가) ·
**두피**(머리카락을 끄면 그 밑에 살이 있는가, 구멍인가). 다 통과하면 종료 코드 0 입니다.

지금 세라·카인·류 맨몸은 네 항목을 전부 통과하고, 아인은 1 번에서 걸립니다.
**색이 살색인지는 검사기가 못 봅니다** — 그건 `viewer.html?body=1` 로 한 번 봐 주세요.
