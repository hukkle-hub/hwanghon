...
위 «연속 재생» 수치는 `mixer.update → restore → 보정` 순서로 잰 것이라, 보정한 팔 뼈가 첫 프레임 값에 멈춘 채였다(게임은 `restore → mixer.update → 보정`).
게임 순서로 다시 재면 이 배포 코드의 연속 재생 최소 간격은 1타 10.6 · 2타 10.9 · 3타 10.8 · 스매시 10.2 · 스킬1 10.5 · 스킬4 11.0 · 처형 11.0 cm — 겹침 없음은 그대로다.
또 게임처럼 재니 오른손 당김·쇄골이 프레임마다 0↔최대로 뒤바뀌어 대검이 튀는 곳이 있었다 → 97 에서 속도 제한.
