...
| 카인 스킬3 «강철 회전» (추가) | Axe_Spin_Attack (238) | 0.20 ~ 2.00 s | 4.00 → **0.91** | 0.90 → 1.50 |
...
- **카인 스킬3**: 원본이 −53° → −410°(357°). 류와 같이 정확히 한 바퀴로 (`--face 1,1.008,-192`).
...
| 류 스킬3 옛 / 새 | 37° / **0°** | 44° / **0°** |
| 카인 스킬3 옛 / 새 | 48° / 95° | 84° / **0°** |

카인은 «시작» 은 섞지 않았다 — 카인 대기는 대검을 쥐는 자세가 공격 자세와 원래 58~146° 다르다.
처음 0.15초를 섞었더니 그 차이를 0.15초에 메우느라 손이 11.9 m/s 로 튀었다(섞기 0.06초가 하는 일을
클립 안에서 두 번 하는 셈). 끝만 섞는다.

### 판정

카인 스킬1 .73 (내려치는 순간), 카인 스킬3 .38 (오른손 최고속), 류 스킬3 .65 (왼날 .62 · 오른날 .72 사이). 곡선화 정책에서
...
2. ~~카인 스킬3(강철 회전) 4.00~~ → 교체함(0.91, 1.50배속).
